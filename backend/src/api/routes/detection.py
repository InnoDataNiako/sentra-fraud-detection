"""
Routes de détection de fraude - Version production
...
@router.post(
    ...
    response_description="Résultat de la détection de fraude"
)
async def detect_fraud(
    ...
    - **fraud_service**: Service de détection de fraude
    
    Retourne le résultat de l'analyse avec score de risque et recommandation
    """
    try:
        ...
        logger.error(f"❌ Erreur inattendue lors de la détection: {str(e)}")
        raise HTTPException(
            status_code=500,
...
@router.post(
    ...
    response_description="Liste des résultats de détection"
)
async def detect_fraud_batch(
    ...
    - **requests**: Liste des transactions à analyser
    - **db**: Session de base de données  
    - **fraud_service**: Service de détection de fraude
    
    Retourne les résultats pour chaque transaction du lot
    """
    try:
        ...
        logger.error(f"❌ Erreur détection par lot: {str(e)}")
        raise HTTPException(
            status_code=500,
